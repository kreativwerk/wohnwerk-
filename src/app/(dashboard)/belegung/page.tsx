...
export default async function OccupancyPage({
  ...
  return (
    <>
      <PageHeader
        title={t("Belegungsplan")}
        description={t("Zeitraum {von} bis {bis}", { von: datum(windowStart), bis: datum(windowEnd) })}
        actions={
          <form className="flex items-center gap-2">
            <select name="objekt" defaultValue={objekt ?? ""} className="min-w-48">
              <option value="">{t("Alle Objekte")}</option>
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4">
        <StatCard label={t("Betten gesamt")} value={String(summary.beds)} />
        <StatCard label={t("Belegt")} value={String(summary.occupied)} tone="brand" />
        <StatCard
          label={t("Frei")}
          value={String(summary.free)}
          tone={summary.free > 0 ? "success" : "neutral"}
          hint={summary.blocked > 0 ? t("{anzahl} gesperrt", { anzahl: summary.blocked }) : undefined}
        />
        <StatCard
          label={t("Miete pro Monat")}
          value={geld(summary.actualRentCents)}
          hint={t("von möglichen {betrag}", { betrag: geld(summary.potentialRentCents) })}
        />
      </div>
      ...
      <div className="mt-6 space-y-6">
        {properties.length === 0 && (
          <EmptyState
            title={t("Keine Objekte vorhanden")}
            description={t("Legen Sie zuerst ein Objekt mit Zimmern und Betten an.")}
            action={
        ...
        {properties.map((property) => {
          ...
          return (
            <Card
              key={property.id}
              title={property.name}
              description={t("{adresse} · {zimmer} Zimmer, {betten} Betten", { adresse: `${property.street}, ${property.zip} ${property.city}`, zimmer: property.rooms.length, betten: bedCount })}
              actions={
            ...
            >
              {bedCount === 0 ? (
                <p className="px-5 py-4 text-sm text-ink-500">
                  {t("Für dieses Objekt sind noch keine Betten angelegt.")}
                </p>
              ) : (
                              ...
                              {bed.tenancies.map((tenancy) => {
                                ...
                                return (
                                  <Link
                                    ...
                                    title={`${tenancy.tenant.firstName} ${tenancy.tenant.lastName} · ${datum(
                                      tenancy.startDate,
      ...
      <div className="mt-4 flex flex-wrap gap-3 text-xs text-ink-500">
        <Badge tone="brand">{t("Aktives Mietverhältnis")}</Badge>
        <Badge tone="info">{t("Vertrag versendet")}</Badge>
        <Badge tone="danger">{t("Bett gesperrt")}</Badge>
        <span className="flex items-center gap-1.5">
          <span className="h-3 w-px bg-ink-900/60" /> {t("heute")}
        </span>
      </div>
    </>
  );
